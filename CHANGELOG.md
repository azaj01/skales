...
- **Declining names the licence you had.** The refusal screen already showed the data folder and the last release under the previous terms; it now says which licence that was - the Business Source Licence 1.1 - and quotes the part of it that has not changed: its Additional Use Grant covered personal use, learning and non-commercial internal testing, so commercial use needed a licence there too. The download link points at skales.app instead of a release tag, because an installed copy can never change the address it shipped with.
