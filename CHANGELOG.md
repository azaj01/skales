...
- **A team of agents started from an incognito chat now stays off the record too.** Ask several agents a question inside an incognito conversation and the run itself behaved: it stayed in memory and refused to file a summary in your documents. Every agent underneath it did not. Each one filed the first hundred characters of your question under recent conversations, pushed up the counter of how long you and Skales have worked together, wrote a line into the activity log that feeds Wrapped, posted to the Discover feed that a conversation had happened, and fed the knowledge graph. A named agent also wrote your question, word for word, into its own run record on disk, and taught itself a lesson from the conversation afterwards. None of that happens off the record now. There is one place in Skales that answers the question "is this off the record", it answers it from the conversation itself and not only from the switch, and everything that runs a turn asks it.

- **The Discover feed and saved images ask the conversation, not the screen.** Both asked only whether the incognito button looked pressed. That button is screen state and a reopened window clears it, while the conversation underneath is still the off-record one - the same gap that made the badge report possible. A picture sent in an incognito chat could still land, at full resolution, in your workspace uploads folder, where the file tools can read it long after the chat is gone.

- **An incognito chat can read its own notes again.** Closing the memory leak took two blocks with it that were never memory: the notes Skales keeps for itself during a conversation, and the summary of a goal it had just finished. Both are read from the conversation, and an incognito conversation lives only in memory and dies with the app, so neither could outlive anything. Worse than the loss: the tool that writes those notes was still switched on, so Skales kept writing notes to itself that it could never read back. Both come back now, for incognito conversations only.

- **Incognito now covers what Skales already knew about you, not just what it would write down.** The badge promised no memory and delivered half of it: the conversation got no file and the memory tools were switched off, so nothing could be looked up - but everything Skales already knew was in the prompt before you typed a word. Your profile, your saved facts, the topic index, the answers you gave on the personalisation card and a line naming what your last conversations were about all travelled into an off-record chat, which is why one could still open by knowing who you were. An incognito conversation now gets the same treatment a remote provider gets under Privacy Mode: language and clock, nothing else. It is told plainly that it is off the record, so it says that instead of inventing a reason for not knowing you, and the receipt under an answer no longer lists memories the reply never saw. A chat you leave and come back to is covered as well - it used to wear the badge on screen and run as an ordinary chat underneath, because coming back forgot which kind it was while the conversation itself had not.

- **An incognito conversation no longer leaves anything behind in your memory.** Every finished turn was recorded: the counter of how long you and Skales have worked together went up, the first hundred characters of your message were filed under recent conversations, and a fact was extracted from what you said - none of which knew that incognito existed. So an off-record chat quietly wrote the very lines that were read back to you in the next one, which is how it could remember a session that was supposed to have left no trace. Nothing from an incognito chat is written now, and the refusal sits at the one place all four channels pass through - the chat, the desktop buddy, WhatsApp and Telegram - rather than at each of them, where it had already been forgotten once.

- **Skales stops inventing a memory about your computer.** Any message containing the word "server", "environment" or "where are you" filed a permanent memory saying it had detected it was running on localhost, on Windows. It had detected nothing: no address, no provider and no operating system were ever read, the sentence was fixed text, and the match was loose enough that "observer", "serverless" and "environment variables" all triggered it - each repeat counting as another confirmation. It was then read back as a fact, and on a Mac or a Linux machine it was simply wrong. The entry is gone, and what it pretended to answer Skales can check properly and on request. The neighbouring rule had the same fault: saying "my name is not important" stored your name as "not", and a fact you asked to have remembered was stored with every occurrence of "that", "this" and "save" cut out of the middle of it. Both now read the sentence you actually wrote.

- **The record of what you recently discussed is being kept again.** Every short note about a finished conversation was silently thrown away before it reached the store, on every machine where the setting for keeping identifiers is off - which is how Skales ships. The check that refuses to store anything shaped like a password was reading the whole record including Skales' own internal conversation number, and that number contains a long run of digits, so the check took our own plumbing for your personal data and discarded the note with it. Only these short notes carried such a number, which is why the memory page kept filling up while the recent-conversation half of it quietly stopped. The check now reads what was said and ignores the envelope it arrived in.

- **The licence now names the cases it used to leave to good manners, and the download carries nothing that could hand its own source back.** It says that Skales is to be run as it was published, that it is not to be built into another program, wrapped, or pointed at from somebody else's tool, that access to it is not to be sold on or bundled, and that nobody may present themselves as a partner of Skales or claim that Skales stands behind their product. Because the text itself changed, every installation is asked once more, with the new points in the summary in all twelve languages and the binding English text one click away - and a further way in which a package can give up the readable original of its finished code is now refused before a release can be built at all.
- **The Apple Silicon download is now built on an Apple Silicon machine.** The release was cut on one computer, and the half of it meant for newer Macs came off a machine of the other kind - which meant that half travelled without the protection its Intel twin had, and nothing in the build said so. The recipe now hands that half to a machine of the right kind, checks the finished package there, brings it back and checks it again here before anything is published. A release that skips that step says out loud what the Apple Silicon package is missing instead of leaving it to be found later.
- **A copy that removes the licence no longer looks untouched.** The record every package keeps of its own files covered the program and its texts, but not the two pages that say who the copy belongs to and what may be done with it - so deleting them or rewriting them left a build that still reported itself as the one that shipped. They are part of that record now: a package that leaves the factory without them is not built at all, and an installation from which they were removed or edited afterwards says so, in the diagnose panel and wherever the build talks to us. It still starts and it still works; it just stops claiming to be an original.
- **Skales now ships under its own licence, and the download says so in one place.** The old terms were borrowed, carried a date on which they would have turned into something else, and left the name and the logo in nobody's terms at all - so somebody who took the published build, put their own name on it and offered it as their own product had nothing in writing to trip over. Skales is proprietary software, licensed and not sold, and it stays that way. Private use, study and teaching stay free of charge, exactly as before; an organisation gets thirty days to try it out; company use, hosting it for other people, white-labelling and passing copies on need a yes in writing first. The licence and a short brand page now stand at the place people actually download from, they travel inside the package next to the third-party notices, and the same two lines go under every release. The open-source licences of the components Skales is built on are unchanged and still carried with it.
- **A Skales build now identifies itself to Skales' own services.** Until now a
  request only said which release was calling, which made an original build and
  a copy of it indistinguishable from the outside. Every build carries its own
  identity from here on and presents it wherever it talks to us. Nothing about
  this is about you: it says which build, never who is using it, and an older
  Skales keeps working exactly as it does today.
- **The downloaded app no longer carries a readable copy of the program it is built from.** Every package Skales has shipped so far came with the app's own uncompiled source alongside the finished program - not needed to run anything, and there for anyone who opened the download. It is gone, while everything the app genuinely reads from disk while running - all twelve languages, the built-in skills, the templates and the design style packs - is untouched. More of the desktop half now ships compiled rather than as plain text, which leaves less of the machinery behind the update and the local engine lying open in a downloaded copy.
- **A Skales install can now tell you whether it is still the build that was published.** Every release writes down what its own packaged files are, and the app checks them once after it starts. When something has been swapped, that is said plainly - in the diagnostics you copy into a bug report, and in the sentence the assistant is given when you ask it why something failed - so a report from a modified copy is no longer mistaken for a report from the real one. A mismatch never blocks the app or hides a crash: it is a statement, not a lock, because a crash report you can read is worth more than a lock. On your own machine, and in a developer checkout, there is nothing new to see.
- **A release build now refuses to ship a package that is not what it should be.** Building a release reads the finished app back and stops if the source crept in again, if the compiled pieces are missing or came with readable copies beside them, if the hardening switches in the binary are not what the build set, or if the record of the packaged files does not match them. Before this, every check of that kind read the source tree - which is not what anyone downloads, and is exactly why the source in the package went unnoticed for months.
- **A copy of Skales that was not built here can now be told apart from one
  that was.** Every release already carried a stamp of its own; the package it
  ships in now also identifies itself as a Skales build in a way that survives
  being taken apart and put back together, and a check before each release
  refuses to build if that identification is no longer intact. None of it is
  visible while you use the app, nothing about you or your machine is part of
